...
| 閉じるボタン   | ✕ ボタン。録音中は確認ダイアログを表示、それ以外は即座に閉じる |
...
✕ボタンの挙動はストリーミングステータスによって異なります。

| ステータス     | 挙動                 |
| -------------- | -------------------- |
| `idle`         | 即座に閉じる         |
| `initializing` | 即座に閉じる         |
| `recording`    | 確認ダイアログを表示 |
| `processing`   | 即座に閉じる         |
| `completed`    | 即座に閉じる         |
| `error`        | 即座に閉じる         |
...
| ---------- | -------------------------------------------------- |
| タイトル   | 「録音を中止しますか？」                           |
| メッセージ | 「録音中のデータは破棄されます。」                 |
| ボタン 1   | 「続ける」（キャンセル、録音継続）                 |
| ボタン 2   | 「中止して閉じる」（破棄してポップアップを閉じる） |

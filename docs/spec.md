...
struct HotKeySettings: Codable, Equatable {
    var recordingHotKey: KeyCombo      // ⌥⇧ Space（録音トグル）
    var pasteHotKey: KeyCombo          // ⌘⇧V（最後の書き起こしをペースト）
    var openSettingsHotKey: KeyCombo   // ⌘⇧,（設定を開く）
    var cancelHotKey: KeyCombo         // Escape（録音キャンセル、固定）
